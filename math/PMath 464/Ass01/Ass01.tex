...
We assume that by a line in $\mathbb A^n(k)$,
...
k$. Thus we can define the sets $S_1'=\{f' \mid f \in S_1\}$ and $S_2''=\{g''
\mid g\in S_2\}$. These are now both sets of polynomials in $\mathbb A^{n+m}(k)$.

Let $I=\langle S_1'\rangle$ and $J=\langle S_2''\rangle$. Then consider the ideal
$IJ$. We claim that $V(IJ)=V\times W$.

To show that $V\times W\subseteq V(IJ)$, let $A\subseteq S_1$ be a subset such
that $V(A)=(a_1,\ldots,a_n)$ and $B\subseteq S_2$ such that $V(B)=(b_1,\ldots,b_
m)$. $A$ and $B$ must exist since $V$ and $W$ are algebraic. Then \[\prod_{f\in
A,g\in B}{f'+g''}\]
